...
static const NSTimeInterval kSlideshowInterval    = 20;
...
-(void) actionSheet: (UIActionSheet *) actionSheet clickedButtonAtIndex: (NSInteger) buttonIndex
{
  if (buttonIndex == 0)
  {
    ...
    [picker setSubject:@"schlijper.nl photo"];
    [picker addAttachmentData:UIImagePNGRepresentation(self.centerPhotoView.image) mimeType:@"image/png" fileName:@"schlijperphoto"];
    [self presentModalViewController:picker animated:YES];
    [picker release];
  }
	else if (buttonIndex == 1)
	{
		UIImageWriteToSavedPhotosAlbum(self.centerPhotoView.image, nil, nil, nil);
	}
}

-(void) photoAction {
  UIActionSheet *as = [ [ UIActionSheet alloc ]
                       initWithTitle:@"Photo actions"
                       delegate:self
                       cancelButtonTitle:@"Cancel"
                       destructiveButtonTitle:nil
                       otherButtonTitles:@"E-mail Photo", @"Save Photo", nil ];
...
- (void)loadView {
  ...
  _actionButton = [ [ UIBarButtonItem alloc ] initWithBarButtonSystemItem:UIBarButtonSystemItemAction target:self action:@selector(photoAction) ];    
  _infoButton = [ [ UIBarButtonItem alloc ] initWithImage:[ UIImage imageNamed:@"info.png" ] style:UIBarButtonItemStylePlain target:self action:@selector(showInfo) ];
  ...
  _toolbar.items = [NSArray arrayWithObjects:
                   _actionButton, space, _previousButton, space, playButton, space, _nextButton, space, _infoButton,nil];
...
- (void)showEmpty:(BOOL)show {
  if (show) {
    ...
    [self showStatus:TTLocalizedString(@"Loading...", @"")];
  } else {
    [self showStatus:nil];
  }
}
